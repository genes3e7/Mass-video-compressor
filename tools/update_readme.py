...
def update_readme(min_version, max_version, file_path="README.md"):
    """
    Update the README's "Prerequisites: Python" line to a specific Python version or a version range.
    
    Replaces the existing "**Prerequisites:** Python ..." line in the target file with either a single version (when min_version == max_version) or a range "min_version - max_version". If the file content changes, writes the updated content back to disk.
    ...
    Raises:
        ValueError: If either version string is empty or does not match the expected numeric format.
        FileNotFoundError: If the specified README file does not exist.
    
    Notes:
        The function may exit the process with status 1 on file read/write errors or if the prerequisites line is not found.
    """
    # 1. Validate Inputs
    ...
    def _replace_match(m):
        """
        Construct a replacement string for a regex match by inserting the module's `version_string` between the preserved surrounding groups.
        
        Parameters:
            m (re.Match): Regex match whose group(1) and group(3) are kept; group(2) is replaced by `version_string`.
        
        Returns:
            str: The new string formed as group(1) + version_string + group(3).
        """
